...
namespace AlipayClass
{
    ...
    public class AlipayConfig
    {
        ...
        public AlipayConfig()
        {
            ...
            partner = System.Configuration.ConfigurationManager.AppSettings["AlipayPartner"];

            //交易安全检验码，由数字和字母组成的32位字符串
            key = System.Configuration.ConfigurationManager.AppSettings["AlipayKey"];

            //签约支付宝账号或卖家支付宝帐户
            seller_email = System.Configuration.ConfigurationManager.AppSettings["AlipaySellerEmail"];

            //付完款后跳转的页面 要用 http://格式的完整路径，不允许加?id=123这类自定义参数
            return_url = "http://" + HttpContext.Current.Request.Url.Authority + "/AlipayReturnUrl";

            //交易过程中服务器通知的页面 要用 http://格式的完整路径，不允许加?id=123这类自定义参数
            notify_url = "http://" + HttpContext.Current.Request.Url.Authority + "/AlipayNotifyUrl";

            //网站商品的展示地址，不允许加?id=123这类自定义参数
            show_url = "http://" + HttpContext.Current.Request.Url.Authority;
